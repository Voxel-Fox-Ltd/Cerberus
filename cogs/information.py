...
class Information(utils.Cog):
    ...
    @commands.command()
    async def points(self, ctx:utils.Context, user:typing.Optional[discord.User], duration:typing.Optional[utils.converters.DurationConverter]={"days": 7}):
        """Tells you how many points a given user has"""

        user = user or ctx.author
        data = utils.CachedMessage.get_messages(user, ctx.guild, **duration)
        await ctx.send(f"{user.mention} has {len(data)} points over {list(duration.values())[0]} {list(duration.keys())[0]}.")

    @commands.command()
    async def leaderboard(self, ctx:utils.Context, duration:typing.Optional[utils.converters.DurationConverter]={"days": 7}):
